...
package com.htm.android_3d.engine.core.services.wavefront;
...
public class WavefrontLoader {
	...
	// flags
	private static final int triangleMode = GLES32.GL_TRIANGLE_FAN;
	...
	public static class Material {
		...
		public void setMaterialColors(GLES32 gl)
		// start rendering using this material's colour information
		{
			...
			// gl.glMaterialfv(GLES32.GL_FRONT_AND_BACK, GLES32.GL_AMBIENT, colorKa, 0);
			// }
			// if (kd != null) { // diffuse color
			// float[] colorKd = {kd.getX(), kd.getY(), kd.getZ(), 1.0f};
			// gl.glMaterialfv(GLES32.GL_FRONT_AND_BACK, GLES32.GL_DIFFUSE, colorKd, 0);
			// }
			// if (ks != null) { // specular color
			// float[] colorKs = {ks.getX(), ks.getY(), ks.getZ(), 1.0f};
			// gl.glMaterialfv(GLES32.GL_FRONT_AND_BACK, GLES32.GL_SPECULAR, colorKs, 0);
			// }
			//
			// if (ns != 0.0f) { // shininess
			// gl.glMaterialf(GLES32.GL_FRONT_AND_BACK, GLES32.GL_SHININESS, ns);
			// }
			//
			// if (d != 1.0f) { // alpha
			// // not implemented
		...
		public boolean addFace(String line) {
			try {
				...
				for (int i = 0, faceIndex = 0; i < numTokens; i++, faceIndex++) {
					...
					if (WavefrontLoader.triangleMode == GLES32.GL_TRIANGLE_FAN) {
						if (faceIndex == 0){
