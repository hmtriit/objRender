package com.htm.android_3d.engine.core.drawer;
...
import com.htm.android_3d.engine.core.model.AnimatedModel;
import com.htm.android_3d.engine.core.model.Object3D;
import com.htm.android_3d.engine.core.model.Object3DData;
import com.htm.android_3d.engine.util.android.GLUtil;
...
class DrawerImpl implements Object3D {
    ...
    private DrawerImpl(String id, String vertexShaderCode, String fragmentShaderCode, Set<String> features) {
        ...
        // load shaders
        int vertexShader = GLUtil.loadShader(GLES32.GL_VERTEX_SHADER, vertexShaderCode);
        int fragmentShader = GLUtil.loadShader(GLES32.GL_FRAGMENT_SHADER, fragmentShaderCode);
    ...
    public void draw(Object3DData obj, float[] pMatrix, float[] vMatrix, int drawMode, int drawSize, int textureId,
        ...
        if (mNormalHandle != -1) {
            GLES32.glDisableVertexAttribArray(mNormalHandle);
        }

        // Disable vertex array
        if (mTextureHandle != -1) {
            GLES32.glDisableVertexAttribArray(mTextureHandle);
        }


        if (mEmissiveTextureHandle != -1) {
            GLES32.glDisableVertexAttribArray(mEmissiveTextureHandle);
        }

        if (in_weightsHandle != -1) {
            GLES32.glDisableVertexAttribArray(in_weightsHandle);
            GLES32.glDisableVertexAttribArray(in_jointIndicesHandle);
        }
    ...
    private void setMvpMatrix(float[] mvpMatrix) {
        ...
        int mMVPMatrixHandle = GLES32.glGetUniformLocation(mProgram, "u_MVPMatrix");
    ...
    private void setColor(Object3DData obj) {
        ...
        int mColorHandle = GLES32.glGetUniformLocation(mProgram, "vColor");
        GLUtil.checkGlError("glGetUniformLocation");
...
//        float[] color = {1f, 0f, 0f, 1f};
    ...
    private int setColors(Object3DData obj) {
        ...
        // get handle to fragment shader's vColor member
        int mColorHandle = GLES32.glGetAttribLocation(mProgram, "a_Color");
        ...
        GLES32.glVertexAttribPointer(mColorHandle, 4, GLES32.GL_FLOAT, false, 0, obj.getVertexColorsArrayBuffer());
    ...
    private int setPosition(Object3DData obj) {
        ...
        int mPositionHandle = GLES32.glGetAttribLocation(mProgram, "a_Position");
        GLUtil.checkGlError("glGetAttribLocation");
        ...
        GLES32.glVertexAttribPointer(mPositionHandle, COORDS_PER_VERTEX, GLES32.GL_FLOAT, false, VERTEX_STRIDE,
                vertexBuffer);
        GLUtil.checkGlError("glVertexAttribPointer");
    ...
    private int setNormals(Object3DData obj) {
        int mNormalHandle = GLES32.glGetAttribLocation(mProgram, "a_Normal");
        GLUtil.checkGlError("glGetAttribLocation");
        ...
        buffer.position(0);
        GLES32.glVertexAttribPointer(mNormalHandle, 3, GLES32.GL_FLOAT, false, 0, buffer);
    ...
    private void setLightPos(float[] lightPosInEyeSpace) {
        int mLightPosHandle = GLES32.glGetUniformLocation(mProgram, "u_LightPos");
        // Pass in the light position in eye space.
        GLES32.glUniform3f(mLightPosHandle, lightPosInEyeSpace[0], lightPosInEyeSpace[1], lightPosInEyeSpace[2]);
    }
    ...
    private void setMvMatrix(float[] mvMatrix) {
        int mMVMatrixHandle = GLES32.glGetUniformLocation(mProgram, "u_MVMatrix");
        GLUtil.checkGlError("glGetUniformLocation");

        // Pass in the modelview matrix.
        GLES32.glUniformMatrix4fv(mMVMatrixHandle, 1, false, mvMatrix, 0);
        GLUtil.checkGlError("glUniformMatrix4fv");
    }
    ...
    private void setColorMask(float[] colorMask) {
        int vColorMaskHandle = GLES32.glGetUniformLocation(mProgram, "vColorMask");
        GLUtil.checkGlError("glGetUniformLocation");

        float[] color = colorMask != null ? colorMask : NO_COLOR_MASK;
        GLES32.glUniform4fv(vColorMaskHandle, 1, color, 0);
        GLUtil.checkGlError("glUniform4fv");
    }
    ...
    private int setEmissiveTexture(Object3DData obj) {
        ...
        int mEmissiveTextureUniformHandle = GLES32.glGetUniformLocation(mProgram, "u_EmissiveTexture");
        GLUtil.checkGlError("glGetUniformLocation");
        ...
        GLES32.glBindTexture(GLES32.GL_TEXTURE_2D, obj.getEmissiveTextureHandle());
        GLUtil.checkGlError("glBindTexture");

        // set sampling and filtering
        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_MIN_FILTER, obj.getEmissiveTextureMinFilter());
        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_MAG_FILTER, obj.getEmissiveTextureMagFilter());
        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_WRAP_S, obj.getEmissiveTextureWrapS());
        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_WRAP_T, obj.getEmissiveTextureWrapT());

        // Tell the texture uniform sampler to use this texture in the shader by binding to texture unit 0.
        GLES32.glUniform1i(mEmissiveTextureUniformHandle, 1);
        GLUtil.checkGlError("glUniform1i");

        int mTextureCoordinateHandle = GLES32.glGetAttribLocation(mProgram, "a_EmissiveTexCoordinate");
        GLUtil.checkGlError("glGetAttribLocation");
        ...
        GLES32.glVertexAttribPointer(mTextureCoordinateHandle, 2, GLES32.GL_FLOAT, false, 0,
    ...
    private int setTexture(Object3DData obj, int textureId) {
        // TODO: add emissive texture
        int mTextureUniformHandle = GLES32.glGetUniformLocation(mProgram, "u_Texture");
        GLUtil.checkGlError("glGetUniformLocation");
        ...
        // set sampling and filtering
        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_MIN_FILTER, obj.getTextureMinFilter());
        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_MAG_FILTER, obj.getTextureMagFilter());
        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_WRAP_S, obj.getTextureWrapS());
        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_WRAP_T, obj.getTextureWrapT());

        // Tell the texture uniform sampler to use this texture in the shader by binding to texture unit 0.
        GLES32.glUniform1i(mTextureUniformHandle, 0);
        GLUtil.checkGlError("glUniform1i");

        int mTextureCoordinateHandle = GLES32.glGetAttribLocation(mProgram, "a_TexCoordinate");
        GLUtil.checkGlError("glGetAttribLocation");
    ...
    private int setWeights(AnimatedModel animatedModel) {
        int in_weightsHandle = GLES32.glGetAttribLocation(mProgram, "in_weights");
        GLUtil.checkGlError("glGetAttribLocation");
        GLES32.glEnableVertexAttribArray(in_weightsHandle);
        GLUtil.checkGlError("glEnableVertexAttribArray");
        animatedModel.getVertexWeights().position(0);
        GLES32.glVertexAttribPointer(in_weightsHandle, 3, GLES32.GL_FLOAT, false, 0, animatedModel.getVertexWeights());
        return in_weightsHandle;
    }

    private int setJoints(AnimatedModel animatedModel) {
        int in_jointIndicesHandle = GLES32.glGetAttribLocation(mProgram, "in_jointIndices");
        GLUtil.checkGlError("glGetAttribLocation");
        GLES32.glEnableVertexAttribArray(in_jointIndicesHandle);
        GLUtil.checkGlError("glEnableVertexAttribArray");
        animatedModel.getJointIds().position(0);
        GLES32.glVertexAttribPointer(in_jointIndicesHandle, 3, GLES32.GL_FLOAT, false, 0, animatedModel.getJointIds());
        GLUtil.checkGlError("glVertexAttribPointer");
    ...
    private void setJointTransforms(AnimatedModel animatedModel) {
        ...
        for (int i = 0; i < jointTransformsArray.length; i++) {
            ...
            int jointTransformsHandle = GLES32.glGetUniformLocation(mProgram, jointTransformHandleName);
            GLUtil.checkGlError("glGetUniformLocation");
            GLES32.glUniformMatrix4fv(jointTransformsHandle, 1, false, jointTransform, 0);
    ...
    private void drawShape(Object3DData obj, int drawMode, int drawSize) {
        ...
        if (!drawUsingUnsignedInt) {
            ...
            drawBufferType = GLES32.GL_UNSIGNED_SHORT;
        }
        ...
        if (drawModeList != null) {
            if (drawOrderBuffer == null) {
                ...
                for (int j = 0; j < drawModeList.size(); j++) {
                    ...
                    if (drawMode == GLES32.GL_LINE_LOOP && polygon[2] > 3) {
                        // is this wireframe?
                        ...
                        for (int i = 0; i < polygon[2] - 2; i++) {
                            ...
                            GLES32.glDrawArrays(drawMode, polygon[1] + i, 3);
                        }
                    } else {
                        GLES32.glDrawArrays(drawMode, polygon[1], polygon[2]);
                    }
                }
            } else {
                ...
                for (int i = 0; i < drawModeList.size(); i++) {
                    ...
                    GLES32.glDrawElements(drawModePolygon, drawSizePolygon, drawBufferType, drawOrderBuffer);
            ...
            if (drawOrderBuffer != null) {
                if (drawSize <= 0) {
                    // String mode = drawMode == GLES32.GL_POINTS ? "Points" : drawMode == GLES32.GL_LINES? "Lines": "Triangles?";
                    // Log.v(obj.getId(),"Drawing all elements with mode '"+drawMode+"'...");
                    drawOrderBuffer.position(0);
                    GLES32.glDrawElements(drawMode, drawOrderBuffer.capacity(), drawBufferType,
                            drawOrderBuffer);
                    if (drawUsingUnsignedInt && GLUtil.checkGlError("glDrawElements")) {
                        drawUsingUnsignedInt = false;
                    }
                    ...
                    for (int i = 0; i < drawOrderBuffer.capacity(); i += drawSize) {
                        ...
                        GLES32.glDrawElements(drawMode, drawSize, drawBufferType, drawOrderBuffer);
                    }
                    if (drawUsingUnsignedInt && GLUtil.checkGlError("glDrawElements")) {
                        drawUsingUnsignedInt = false;
                    }
